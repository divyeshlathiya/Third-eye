...
google_dummy_pass = os.getenv("GOOGLE_PASS", "google_default_secret")
...
async def auth_firebase(data: TokenRequest, db: Session = Depends(get_db)):
    ...
    try:
        ...
        if not user:
            user = User(
                ...
                gender=gender,
                profile_pic=profile_pic,
                password=hash_password(google_dummy_pass),
                provider="google",
            )
            db.add(user)
            db.commit()
    ...
    access_token = create_access_token(data=payload, expire_delta=timedelta(minutes=30))
    refresh_token = create_refresh_token(data=payload, expire_delta=timedelta(days=7))
